...
// other; comb is the hand, and the warp it works with never repeats. A
// transient is a fresh drop — a ring that spreads and thins as the envelope
// dies. The arc is the ink sinking: lines swell, blur and let go of their
// figure until the bath is dark water again.
const fragmentShader = /* glsl */ `
  precision highp float;
  ${glslCommon}
  uniform float uTime;
  uniform vec2 uResolution;
  uniform float uFog, uFlow, uDepth, uDark, uComb, uDrop;
  varying vec2 vUv;

  void main(){
    float aspect = uResolution.x / max(uResolution.y, 1.0);
    vec2 pc = (vUv - 0.5) * vec2(aspect, 1.0);
    float t = uTime * (0.03 + uFlow * 0.10);

    vec2 p = pc * mix(1.2, 2.6, uDepth);

    // the stylus: an organic warp dragged through the bath — the marbling hand
    float combAmp = uComb * (0.55 + 0.25 * sin(uTime * 0.021));
    vec2 w = vec2(fbm3(p * 1.35 + vec2(t, -t * 0.6)),
                  fbm3(p * 1.35 + vec2(7.7, 2.9) + vec2(-t * 0.8, t)));
    vec2 q = p + combAmp * 1.5 * (w - 0.5) * 2.0;

    // two drop-points breathing against each other
    vec2 c0 = 0.34 * vec2(sin(uTime * 0.031), cos(uTime * 0.024));
    vec2 c1 = -0.30 * vec2(cos(uTime * 0.019), sin(uTime * 0.027));
    ...
    float ink0 = 1.0 - smoothstep(soft, soft + 0.10, abs(fract(f0) - 0.5));
    float ink1 = 1.0 - smoothstep(soft, soft + 0.10, abs(fract(f1) - 0.5));
    float ink = max(ink0, ink1 * 0.75);

    // a fresh drop rings outward and thins as the envelope dies
    vec2 cD = 0.22 * vec2(sin(uTime * 0.013 + 4.0), cos(uTime * 0.017 + 1.0));
    float rp = (1.0 - uDrop) * 1.1;
    float front = exp(-abs(length(q - cD) - rp) * 22.0) * uDrop * uDrop;

    vec3 water = vec3(0.010, 0.011, 0.014);
    vec3 indigo = vec3(0.40, 0.43, 0.55);
    vec3 col = water
             + indigo * ink * 0.55 * (1.0 - 0.72 * uDark)
