...
// whatever is out there answering as short-lived blips that fade until the
// beam comes round again. A transient fires the array: a wavefront rolls
// outward and everything it crosses flares. Gain is how hard you listen — how
// much of the dark answers back. The arc is the set losing power: the returns
// die first, then the range rings, and last a ghost of the beam turning in a
// dead display.
const fragmentShader = /* glsl */ `
  ...
  uniform float uFog, uFlow, uDepth, uDark, uPing, uGain;
  varying vec2 vUv;
  ...
  void main(){
    ...
    // the returns: sparse contacts per polar cell, re-rolled each revolution
    float Na = 48.0;
    ...
    vec2 seed2 = hash2(vec2(cellA * 1.7 + 13.1, cellR * 3.1 + rev * 17.3));
    float lives = step(1.0 - 0.045 * (0.25 + uGain), seed2.x) * step(0.06, r) * step(r, 1.05);
    vec2 blipPos = vec2(cos(aC), sin(aC)) * ((cellR + 0.2 + 0.6 * seed2.y) / Nr);
    float dB = length(pc - blipPos);
    float lagC = fract((sw - aC) / TAU);
    float blip = lives * exp(-dB * dB * 2600.0) * exp(-lagC * 3.2);
    ...
    float speckle = step(0.9975 - uGain * 0.0018, hash(floor(pc * 130.0) + floor(uTime * 3.0)))
                  * trail * 0.5;
