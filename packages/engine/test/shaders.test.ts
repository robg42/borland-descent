...
/** Module-level `const NAME = 192;` — the stateful sims bake their simulation
 *  resolutions into the GLSL rather than paying for a uniform read per texel. */
const NUMERIC_CONST = /^const\s+([A-Za-z_]\w*)\s*=\s*(-?\d+(?:\.\d+)?)\s*;/gm;

/** `${RES}` and `${RES.toFixed(1)}` against those constants; nothing else. */
function substituteNumericConsts(glsl: string, ts: string): string {
  let out = glsl;
  for (const m of ts.matchAll(NUMERIC_CONST)) {
    const [, name, literal] = m as unknown as [string, string, string];
    const value = Number(literal);
    out = out.replaceAll(`\${${name}}`, literal);
    out = out.replace(
      new RegExp(`\\$\\{${name}\\.toFixed\\((\\d+)\\)\\}`, 'g'),
      (_, digits: string) => value.toFixed(Number(digits)),
    );
  }
  return out;
}
...
function extractFragmentShaders(file: string): ShaderSource[] {
  ...
  for (const m of ts.matchAll(FRAGMENT_LITERAL)) {
    const name = m[1]!;
    const glsl = substituteNumericConsts(m[2]!.replaceAll('${glslCommon}', glslCommon), ts);
